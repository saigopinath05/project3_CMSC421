...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for T in self.mdp.getTransitionStatesAndProbs(state, action):
            #print(T)
            curr, p = T
            q += p * (self.mdp.getReward(state, action, curr) + self.discount*self.values[curr])
        return q
...
class AsynchronousValueIterationAgent(ValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        for i in range(self.iterations):
            ...
            q = self.computeQValueFromValues(state, action)
            #new[state] = q
            #print("l of values: ", len(self.values))
            self.values[state] = q
            idx+=1
...
class PrioritizedSweepingValueIterationAgent(AsynchronousValueIterationAgent):
    ...
    def runValueIteration(self):
        "*** YOUR CODE HERE ***"
        predecessors = dict.fromkeys(self.mdp.getStates(), set())
        for state in self.mdp.getStates():
            for action in self.mdp.getPossibleActions(state):
        ...
        for state in self.mdp.getStates():
            if not self.mdp.isTerminal(state):
                max = self.getMaxQ(state)
                diff = abs(self.values[state] - max)
                prio.push(state, diff*-1)
